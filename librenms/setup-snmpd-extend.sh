            ...
            if [ ${#Extend_OID} -gt 0 ]; then
                if [ ${Install} -eq 0 ]; then
                    ...
                    if [ $? -eq 0 ]; then
                        ...
                        if [ $? -eq 0 ]; then
                            _echo "extend ${Extend_OID} ${Extend_Name} installed." OK
                        else
                            _echo "extend ${Extend_OID} ${Extend_Name} failed to install." WARNING
                        fi
                    fi
                else
                    _echo "extend ${Extend_OID} ${Extend_Name} returns success." OK
                fi
            fi
        fi
        unset -v Extend_OID
...
while read Extend_Check; do
    ...
    chmod 0700 "${Extend_Check}" &> /dev/null
    ...
    if [ ${Extend_RC} -eq 0 ]; then
        if [ ${Install} -eq 0 ]; then
            ...
            if [ $? -eq 0 ]; then
                ...
                if [ $? -eq 0 ]; then
                    _echo "extend ${Extend_Name} installed." OK
                else
                    _echo "extend ${Extend_Name} failed to install." WARNING
                fi
            fi
        else
            _echo "extend ${Extend_Name} returns success." OK
        fi
    else
        if [ ${Extend_RC} -eq 126 ]; then
            _echo "extend ${Extend_Name} failed; permission denied. (rc=${Extend_RC})." WARNING
            ls -l "${Extend_Check}"
        fi
    fi
    unset -v Extend_Exec_Args Extend_Basename Extend_Name Extend_RC
done <<< "$(grep -l -m1 "^#!" "${Dirname}"/extend-* | egrep -ve "/${Basename}|/extend-info|cfg$|conf$|env$")"
